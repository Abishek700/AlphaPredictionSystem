...
%%%%%%
\chapter{Deployment}
\label{chapter:deplyment}
Before describing the technical implementation of the deployment, it is useful to illustrate how the deployed system operates from a user perspective. Figure \ref{fig:deployment_workflow} illustrates the overall workflow 
of the deployed Alpha Recommendation System, showing how the 
trained model, contextual adjustments, and the user interface 
interact to produce a final $\alpha$ recommendation.
\begin{figure}[h]
	\centering
	\begin{tikzpicture}[
		node distance=1.5cm,
...
The trained model developed in Chapter\ref{chapter:development} is integrated into a Streamlit application. The following sections describe how users interact with the system to obtain an alpha recommendation.


\subsection{Purpose of Deployment}

The purpose of deployment is to make the developed model usable for researchers during the study planning stage. Instead of running Python scripts manually, users interact with the system through a simple interface for entering study parameters and receive a recommended significance level.
...
After the machine learning model is trained during system development, it is saved as a serialized \texttt{.pkl} file using the Python \textit{pickle} library. This file stores the trained model and its learned parameters.
...
The interaction between the stored model, the Streamlit interface, and the prediction pipeline is illustrated in Figure~\ref{fig:prediction_workflow}
...
\begin{itemize}
\item First, the trained machine learning model predicts a $\alpha_{base}$ value using the input sample size and expected effect size.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		\node (predict) [below of=code] {Model Predicts $\alpha_{base}$};
...
In the example shown, a study with a sample size of 241, an expected effect size of 0.30, a lenient risk tolerance, a one tailed test, and one hypothesis test results in a recommended alpha value of 0.0747. 

To illustrate how this value is derived, consider the step-by-step calculation. The trained model first predicts a baseline value of $\alpha_{base}$ = 0.030 based on the sample size of 241 and effect size of 0.30. In the second stage, the lenient risk tolerance adjustment multiplies this value by 1.25, giving 0.030 × 1.25 = 0.0375. Since a one-tailed test was selected, the value is further multiplied by 2, giving 0.0375 × 2 = 0.075. Finally, as only one hypothesis is tested, no Bonferroni correction is applied. The result is rounded to four decimal places and clipped to the range [0.001, 0.15], producing the final recommended alpha of 0.0747. Each step corresponds directly to a researcher decision, making the recommendation fully transparent and reproducible.This value is generated through the predictive model and subsequent contextual adjustments.

After the $\alpha_{base}$ value has been computed, the application must communicate the result to the user through the interface. The following code implements the output component of the deployed application. It retrieves the final alpha recommendation generated by the prediction pipeline and displays it in the Streamlit interface. This step completes the interaction cycle by presenting the result of the model prediction and contextual adjustments in a clear and accessible format for the user.
...
When the application starts, Streamlit automatically opens the interface in a web browser. The user can then enter the required study parameters, such as sample size, effect size, risk tolerance, test type, and number of hypothesis tests.After providing the inputs and clicking the recommendation button, the system generates and displays the recommended alpha value.This setup allows the Alpha Recommendation System to be easily executed and reused by other researchers.

The source code for the system is available in the project repository \url{https://github.com/Abishek700/AlphaPredictionSystem}. Additional information about the repository and instructions for running
the application are provided in Appendix\ref{sec:gitappendix}.

\section{Summary}

The deployed Streamlit application provides a complete and 
self contained tool for $\alpha$ recommendation. Users with no 
programming experience can obtain a context adjusted significance 
level by entering five study parameters through a simple 
interface. The following chapter evaluates whether the 
recommendations produced by this system would have supported 
more reliable statistical decisions compared to the conventional 
