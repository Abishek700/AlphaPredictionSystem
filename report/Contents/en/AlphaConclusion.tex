...
This thesis addressed the problem of using a fixed significance level, commonly $\alpha = 0.05$, in hypothesis testing. Although widely used, this threshold is often applied without considering the specific characteristics of a study. Such uniform use may contribute to misleading statistical decisions and replication failures\cite{Benjamin2018}\cite{WassersteinLazar2016}.

To address this issue, a data-driven alpha recommendation system was developed. The system uses replication data to learn patterns between sample size, effect size, and study outcomes. A Random Forest regression model was trained to estimate a baseline alpha value based on these study characteristics. In addition to this data-driven component, contextual adjustments were incorporated to account for user preferences such as risk tolerance, test direction, and multiple hypothesis testing.
...
The proposed system was applied to 20 failed replication studies drawn from multiple research fields. These studies originally reported statistically significant results using $\alpha = 0.05$ but later failed to replicate\cite{FORRTFReDExplorer}.

The results showed that the recommended alpha values varied across studies rather than remaining fixed at 0.05. The system first predicts a baseline alpha value using a machine learning model trained on patterns observed in previous studies, and then adjusts this value based on contextual study characteristics.
 
In 11 out of 20 cases, the system suggested a stricter alpha level, which would have prevented the rejection of the null hypothesis in studies that later failed to replicate. This demonstrates that combining data-driven prediction with context-dependent adjustment can reduce potentially misleading statistical decisions.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/MainfindingsComparision}
...
Fourth, the proposed system does not determine sample size and does not replace traditional power analysis. Instead, it assumes that important study parameters, such as sample size and expected effect size, are already specified by the researcher.
...
First, the contextual scaling factors used in this study could be further refined using formal decision-theoretic models. Developing such models would require a deeper theoretical analysis and empirical calibration across many studies, which is beyond the scope of the present work.
