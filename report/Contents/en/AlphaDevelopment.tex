...
The main contribution of this thesis is the development of a data driven system for recommending study specific significance levels.The proposed system combines empirical evidence from replication studies with help of machine learning followed by contextual statistical adjustments. This approach allows the recommended significance level to adapt to study characteristics and research preferences.
...
\label{sec:overview}
  The recommendation is intended to guide decision making during the study design stage and does not replace statistical judgment.
...
\label{sec:Dataset}
This section explains how the dataset used to train ML model in the Alpha Recommendation System is constructed. It describes the data sources, their reliability, the procedure used to define optimal alpha values, the methods used to recover missing sample sizes and effect sizes, and the structure of the final dataset.
...
dataset. Values 0.07 and 0.08 were not observed as optimal thresholds in the dataset and were therefore excluded from the candidate set. The optimal alpha values observed across all 664 studies fell within this discrete range, making these values the natural choice for the candidate set.

This equation (Equation~\ref{eq:optalpha}) was derived specifically for this system to operationalise the concept of a study-specific threshold. While the general idea of 
...
Here, $\mathcal{A}$ denotes a predefined set of candidate significance levels. This rule ensures that the original result would not have been declared statistically significant under $\alpha_{opt}$, while the chosen value remains interpretable and comparable across studies.
...
To ensure consistency across studies in the dataset, each hypothesis test must be associated with a sample size and an effect size. When these values were not reported directly, they were recovered using standard statistical formulas.
...
To ensure transparency and reproducibility, the complete dataset and all source code used for data processing, model development, and system implementation are provided in the accompanying GitHub repository (see Appendix \ref{sec:gitappendix}).

\section{ML Model for Predicting $\alpha_{base}$ }
\label{sec:MLpart}
Machine learning refers to a class of computational methods that learn patterns from data rather than following explicitly programmed rules\cite{Jordan2015}. In this system, a machine learning model is used because the relationship between sample size, effect size, and appropriate significance levels is not known in advance and cannot be expressed as a simple formula it must be learned from empirical replication data.
...
While the previous section described how the dataset was built and how optimal alpha values were defined\ref{sec:Dataset}, this section focuses on processing those data for machine learning and implementing the predictive model.
...
Since the objective is to predict a continuous optimal alpha value, the problem is formulated as a regression task. A regression algorithm learns a function that maps input variables to a continuous numerical output. Three candidate algorithms were evaluated. A Decision Tree regressor partitions the input space into regions and assigns a constant prediction to each region. An  XGBoost regressor builds trees sequentially, where each new tree corrects the residual errors of the previous one. A Random Forest regressor builds a large number of independent decision trees and combines their predictions by averaging, which reduces variance and improves generalization\cite{Breiman2001}. 

To evaluate predictive performance, the dataset was divided into training and testing subsets. Seventy percent of the observations were used to train the models, while the remaining thirty percent were reserved for testing. Model performance was evaluated on the test set using the Mean Absolute Error (MAE) metric. Their predictive performance was compared using MAE \ref{fig:DecisonTree}, \ref{fig:GradientBoosting},\ref{fig:RandomForest}.
...
MAE measures the average absolute difference between the predicted and actual values. This metric was chosen because it is easy to understand and expresses the prediction error in the same units as the target variable. In addition, MAE is less sensitive to large errors than metrics such as Mean Squared Error (MSE), since it does not square the differences between predicted and actual values \cite{WillmottMatsuura2005}.
\begin{figure}[H]
	\centering
...
The Random Forest model is an ensemble learning method that combines multiple decision trees to improve prediction accuracy\cite{Breiman2001}. 

The Figure\ref{fig:randomforestregressor} explains the working of the model, First the training dataset is used to generate multiple random samples through a process called bootstrap sampling. Each sample is then used to train an individual decision tree independently.

Each decision tree makes its own prediction based on the input features. Since the trees are trained on slightly different data, their predictions may vary.

Finally, the predictions from all trees are combined by averaging them to produce the final prediction. This approach reduces the impact of noise and improves the stability and accuracy of the model\cite{Ha2023}.
...
The Random Forest regression model was implemented using the \texttt{scikit-learn} library in Python. The dataset was divided into training and test sets using an 70/30 split. This proportion is commonly used in machine learning because it balances the need for sufficient training data with the need for an independent evaluation dataset. The number of trees in the forest was set to 500. Increasing the number of trees generally improves the stability and predictive performance of ensemble models, although the gains diminish beyond a certain point\cite{Breiman2001}. A higher number of estimators was selected to ensure robust and stable predictions. The maximum depth of each tree was limited to 30 in order to control model complexity and reduce the risk of overfitting. The random state was fixed at 42 to ensure reproducibility of the results. In addition, parallel processing was enabled using \texttt{n\_jobs = -1} to utilize all available CPU cores and improve computational efficiency\cite{scikit-learn2023}.
...
\end{lstlisting}
After training, the model is used to predict a baseline significance level ($\alpha_{\text{base}}$) for new inputs. Given input values such as sample size and expected effect size, the model returns a predicted alpha value based on patterns learned from the replication data.
...
The overall adjustment method, discussed in Section~\ref{sec:overview}, 
...
The adjustment is implemented in Python as:
\begin{lstlisting}[language=Python]
	# Risk Tolerance
...
The $\alpha_{final}$ value should be understood as a recommendation rather than a strict rule. It combines information learned from replication data with preferences defined by the researcher. The predictive model first estimates a $\alpha_{base}$ based on patterns observed in previous studies.
