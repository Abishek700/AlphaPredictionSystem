...
The system is implemented as a software application in Python and includes a machine learning component. It is explained in two stages for quick understandings. In the first stage, a data driven regression model is trained on replication data to estimate a baseline $\alpha$ value($\alpha_{base}$). The model uses sample size and effect size as input variables, since these quantities are known before data collection and directly influence statistical power and error probabilities\cite{PetersonFoley2021}.

In the second stage, the baseline $\alpha$ value is adjusted to account for study and user preferred considerations. These adjustments are based on information provided by the researcher, including risk tolerance, test directionality, and the number of hypotheses being tested. The adjustments follow predefined and transparent rules to ensure interpretability\cite{Herzog2019}\cite{Hales2023}\cite{Baker2016}.
...
The system produces a final recommended $\alpha$ value before hypothesis testing is conducted. It does not use calculated $p$-values or study results and does not attempt to define a theoretically optimal significance level. Instead, it provides a practical and context-sensitive reference value to support informed study design decisions. The overall structure of the Alpha Recommendation System is illustrated in Figure~\ref{fig:alpha-flow}.
...
The internal data preparation and model training procedure used to obtain this baseline $\alpha$ value are described in a later section \ref{sec:Dataset}\ref{sec:MLpart}.

\subsection{Context-Based Adjustment of Alpha}

The baseline $\alpha$ value produced in the first stage does not include all aspects of a study. Accurate decisions depend on the research context and the researcher’s preferences in addition to the patterns observed in replication data which provide only a numerical baseline and do not account for the specific context or preferences of the researcher. For this reason, the Alpha Recommendation System includes a second stage that adjusts the baseline $\alpha$ value using user-defined inputs.
...
The system applies simple and predefined rules to modify the baseline $\alpha$ value. Stricter risk preferences lead to smaller $\alpha$ values, while more lenient preferences allow larger values\cite{Baker2016}.

The first adjustment modifies the baseline alpha according to the selected level of risk tolerance
\begin{equation}
	\alpha' 
	= 
	\alpha_{\text{baseline}} \cdot k_1
\end{equation}
...
Although only three contextual factors are included, the system remains broadly usable. The baseline $\alpha$ value, which is derived from replication data, can serve as a default and data-informed reference for selecting the significance level. The contextual adjustments are optional refinements that allow the recommendation to better reflect specific study preferences. Even if additional contextual factors are not included, the system still provides a more flexible and evidence-based alternative to using a fixed value such as $\alpha = 0.05$.
...
Replication data were collected from five large-scale projects hosted on the Open Science Framework (OSF). 

\begin{enumerate}
	
	\item \textbf{Reproducibility Project: Cancer Biology} \\
	Focus: Replication of biomedical findings. \\
	Data extracted: Original $p$-values, standardized effect sizes (SMD), sample sizes, and replication $p$-values \cite{ErringtonDenis2021}.
	
	\item \textbf{Challenging the N-Myopia} \\
	Focus: Large-scale psychology replication efforts. \\
	Data extracted: Original $p$-values, standardized effect sizes, sample sizes, and replication success indicators. \\
	Purpose: To examine predictors of replicability in psychological research \cite{Li2021}.
	
	\item \textbf{Applying Z-Curve to Large-Scale Replication Studies} \\
	Focus: Statistical evaluation of replication datasets. \\
	Data extracted: Effect sizes and sample sizes recovered from reported test statistics ($t$, $F$, $\chi^2$), along with replication outcomes. \\
	Purpose: To expand the dataset while maintaining statistical consistency\cite{Sotola2022}.
	
	\item \textbf{Pre-hacking Project} \\
	Focus: The impact of analytical flexibility on research findings. \\
	Data extracted: Original $p$-values, recovered effect sizes, sample sizes, and replication results. \\
	Purpose: To analyze how design decisions may influence statistical significance\cite{Chang2025}.
	
	\item \textbf{Life Outcomes of Personality Replication (LOOPR)} \\
	Focus: Preregistered, high-powered replications of personality--outcome associations. \\
	Data extracted: Outcome-level $p$-values, effect sizes, sample sizes, and replication success indicators\cite{Soto2017}.
...
For studies in which the replication does not confirm the original finding, the original significance level is considered too lenient. In these cases, an alternative $\alpha$ value is calculated using the original study’s reported $p$-value. The optimal alpha is defined as the largest significance level that is smaller than the original $p$-value. This ensures that the original result would not have been declared statistically significant under this adjusted threshold.
...
\label{sec:MLpart}
This section explains how the constructed dataset is processed specifically for model training, how the input features and target variable are selected, and how the data-driven model is implemented to estimate a baseline significance level ($\alpha$).
...
When new study inputs are provided, the model produces a baseline alpha value based on pattern learned from similar past cases.
...
Since the objective is to predict a continuous optimal alpha value, the problem is formulated as a regression task. Several machine learning algorithms were tested for this purpose, including Decision Tree, Random Forest, and XGBoost regression models. Their predictive performance was compared using standard evaluation metrics \ref{fig:DecisonTree} \ref{fig:GradientBoosting}\ref{fig:RandomForest}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.62\textwidth]{Images/pred_vs_actual_decision_tree}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.62\textwidth]{Images/pred_vs_actual_gradient_boosting}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.62\textwidth]{Images/pred_vs_actual_random_forest}
	\caption{Random Forest Regressor: Predicted/Actual  Alpha Values}
	\label{fig:RandomForest}
\end{figure}

Among the tested models, the Random Forest regression algorithm achieved the most stable and accurate results on the dataset. Random Forest builds multiple decision trees and combines their predictions by averaging \cite{Breiman2001}. This approach better
...
After training, the model can predict a baseline alpha for new inputs. When a sample size of 150 and an expected effect size of 0.30 are provided, the model returns a predicted alpha value based on patterns learned from the replication data.

\subsubsection*{Predicting a Baseline Alpha}
...
\section{Context-Based Adjustment of the Baseline Alpha}
\label{sec:ContextFactors}
This section describes the second stage of the Alpha Recommendation System. In this stage, the baseline alpha value produced by the data-driven model is adjusted using user-defined preferences with standard statistical rules. The purpose of this step is to update baseline alpha to become context aware and to suggest user an alpha to their preference.

\subsection{Motivation for Context-Based Adjustment}

The baseline alpha predicted by the model reflects patterns observed in replication studies. However, no single alpha value can be appropriate for all research situations. Even when sample size and effect size are similar, studies may differ in risk tolerance, testing strategy, or research goals .
...
Although other contextual factors may exist, the system focuses on these widely used adjustments to maintain simplicity and clarity. The baseline alpha remains usable as a general reference even without additional adjustments.
...
\begin{equation}
	...
	\alpha_{\text{baseline}} \cdot k_1
\end{equation}
Each level is converted into a numerical adjustment by multiplying the baseline alpha with a fixed scaling factor. This converts a qualitative choice (for example, “strict”) into a clear mathematical change.
...
Multiplication is used because alpha is a probability value. Proportional changes keep the adjustment consistent across different baseline values. For example, if the baseline alpha is $0.04$:
...
The scaling values ($0.5$, $0.75$, $1.25$, $1.5$) are heuristic choices. They are selected to represent clear and understandable differences in strictness. They are not derived from a formal theoretical model. Instead, they are practical design parameters that translate qualitative risk preferences into transparent numerical adjustments.

\subsection{Test Directionality}

After adjusting the baseline alpha for risk tolerance, the system next considers the direction of the hypothesis test.
...
The final alpha value is obtained by applying the adjustment rules sequentially to the baseline alpha:
...
To prevent unrealistic values, the final alpha is restricted to a predefined range:
...
The final alpha value should be understood as a recommendation rather than a strict rule. It combines information learned from replication data with preferences defined by the researcher. The predictive model first estimates a baseline alpha based on patterns observed in previous studies.
...
The context-based adjustment represents the second and final stage of the Alpha Recommendation System. In the first stage, the machine learning model predicts a baseline alpha using empirical data. In the second stage, the researcher can modify this value to reflect study-specific considerations.
