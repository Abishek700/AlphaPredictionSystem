...
\lstdefinestyle{pythonstyle}{
	language=Python,
	basicstyle=\ttfamily\small,             % Use small font for better readability
	keywordstyle=\color{teal}\bfseries,     % Teal for Python keywords
	stringstyle=\color{orange},             % Orange for strings
	commentstyle=\color{green!50!black}\itshape, % Greenish color for comments (including DoXigen)
	showstringspaces=false,                 % No visible spaces in strings
	numbers=left,                           % Line numbers on the left
	numberstyle=\scriptsize\color{gray},    % Small gray font for line numbers
	stepnumber=1,                           % Show line numbers for each line
	numbersep=10pt,                         % Space between line numbers and code
	breaklines=true,                        % Allow long lines to wrap
	frame=single,                           % Use a single-line border around the code
	frameround=tttt,                        % Rounded corners for the frame
	rulecolor=\color{black},                % Black frame for professionalism
	backgroundcolor=\color{lightgray!10},   % Light gray background for contrast
	captionpos=b,                           % Place captions below the code
	tabsize=4,                              % Set tab width to 4 spaces
	morekeywords={self, as, with},          % Add custom Python keywords
	emph={True, False, None},               % Emphasize Python booleans and `None`
	emphstyle=\color{purple}\bfseries       % Style for emphasized words
}